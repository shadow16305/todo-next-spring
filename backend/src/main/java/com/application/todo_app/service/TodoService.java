package com.application.todo_app.service;
...
import com.application.todo_app.repository.TodoRepository;
...
public class TodoService {
    ...
    public Todo updateTodoItem(Integer id, Todo todoItem) {
        Optional<Todo> todoOpt =
                todoRepo.fetchAllTodos().stream().filter(item -> item.getId().equals(id)).findAny();
